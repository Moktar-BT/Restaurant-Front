...
function Login() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('/auth/signin', { // Update the endpoint to /auth/signin
        email,
        password,
      });
    ...
    <div className={styles.container}>
      ...
      <div className={styles.rightcontainer}>
        ...
        <form className={styles.form} onSubmit={handleSubmit}>
          ...
          <div className={styles.form_group}>
            <input type="submit" value="Log in" className={styles.submit_button} />
          </div>
          <div className={styles.signup_link}>
            <p>Don't have an account? <Link to="/Signup" className={styles.ins}>Sign up here.</Link></p>
          </div>
        </form>
      </div>
    </div>
